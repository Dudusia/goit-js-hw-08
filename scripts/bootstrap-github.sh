...
# Function to check if Github pages are enabled
check_github_pages() {
    # Make the API request to get GitHub Pages information
...
enable_github_pages() {
    response=$(curl -s -o /dev/null -w "%{http_code}" \
        ...
        -d "{\"build_type\": \"workflow\", \"source\": {\"branch\": \"$BRANCH_NAME\", \"path\": \"/\"}}" "$PAGES_URL")
    if [ "$response" = "201" ]; then
        echo "✅ GitHub Pages enabled for $REPO_NAME from branch $BRANCH_NAME."
    else
...
if check_github_pages; then
    echo "🧿 Github pages are enabled"
else
    echo "Github pages are disabled, enabling..."
    enable_github_pages
fi
